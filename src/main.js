document.addEventListener("DOMContentLoaded", () => {
  ...
  function createChatroomItem(chatroom, contact, lastTime) {
    ...
    chatroomItem.setAttribute(
      "data-messages",
      JSON.stringify(chatroom.messages.map((msg) => ({ body: msg.body })))
    );
      ...
      if (chatroom.hasMedia) {
        ...
        chatroomItem.innerHTML = `
                    ...
                    <div class="row w-100">
                        ...
                        <p class="message-preview mb-1">${formatMsg(
                          chatroom.last_chat
                        )}</p>
                    </div>
                `;
  ...
  function updateDraftIndicator(chatroom) {
    ...
    if (chatroom.classList.contains("selected")) {
      chatroom.querySelector(".message-preview").textContent =
        formatMsg(lastChat);
    } else {
      ...
      if (draft) {
        chatroom.querySelector(
          ".message-preview"
        ).innerHTML = `<span style="color: #15976e;">Draft: </span>${draft.trim()}`;
      } else {
        chatroom.querySelector(".message-preview").textContent =
          formatMsg(lastChat);
      }
  ...
  function formatLastChatTime(timestamp) {
    ...
    const diffDays = Math.floor(
      (currentDate - lastChatDate) / (1000 * 3600 * 24)
    );
  ...
  function setupChatHeader(contactName) {
    ...
    searchMessage.addEventListener("input", function (e) {
      ...
      separators.forEach((separator) => {
        ...
        while (
          nextElement &&
          !nextElement.classList.contains("time-separator")
        ) {
  ...
  chatInputField.addEventListener("input", function () {
    ...
    if (chatInputField.value !== "") {
      ...
      const selectedChatroom = document.querySelector(
        `[data-remote-id="${currentChatroomID}"]`
      );
      if (selectedChatroom) {
  ...
  async function displayChatHistory(messages) {
    ...
    const lastChatTime = formatLastChatTime(
      messages[messages.length - 1].timestamp
    );
    document.querySelector(
      ".chat-header .last-time"
    ).textContent = `Last chat on ${lastChatTime}`;
      ...
      if (currentChatroomID.includes("@g.us") && !msg.fromMe) {
        ...
        messageContainer.innerHTML = `
                    <div class="message received d-flex justify-content-start">
                        <div class="d-flex">
                            <img src="./img/default-profile-picture-01.png" alt="${formatContactName(
                              msg.author,
                              contacts
                            )}'s profile picture" class="profile-chat">
                        </div>
                        <div class="d-flex flex-column">
                            <div class="notify-name">${formatContactName(
                              msg.author,
                              contacts
                            )}</div>
                            <div class="message-bubble">
                                <div class="message-body">${formatMsg(
                                  msg.body
                                )}<div>
                                <small class="timestamp">${formatTimestamp(
                                  msg.timestamp
                                )}</small>
                            </div>
                        </div>
        ...
        if (msg.hasQuotedMsg) {
          ...
          if (msg._data.quotedParticipant === thisUserID) {
            ...
            quotedParticipant = formatContactName(
              msg._data.quotedParticipant,
              contacts
            );
          }
        ...
        if (msg.hasQuotedMsg) {
          ...
          quotedMessage.innerHTML = `
                        <div class="notify-name">${
                          ...
                          msg._data.quotedParticipant.split("@")[0]
                        }</div>
                        ${formatMsg(msg._data.quotedMsg.body)}
                    `;
  ...
  searchContact.addEventListener("input", function () {
    ...
    document.querySelectorAll(".contact-list").forEach((contact) => {
      ...
      const messages = JSON.parse(
        contact.getAttribute("data-messages") || "[]"
      ); // Safely parse data-messages

      // Check if contact name or any associated message matches the query
      const isNameMatch = contactName.includes(searchQuery);
      const isMessageMatch = messages.some((message) =>
        message.body.toLowerCase().includes(searchQuery)
      );
  ...
  async function sendMessage() {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Error sending message: ${errorDetails.error || response.statusText}`
        );
      }
  ...
  function handleFiles(files) {
    ...
    document
      .querySelector(".btn-clear-preview")
  ...
  async function sendFile() {
    ...
    if (mediaInput.files.length > 0 && currentChatroomID != null) {
      ...
      const messageContent = document
        .getElementById("caption-input-form")
        .value.trim();
  ...
  function createFileElement(files, messageBubble, messageDiv) {
    ...
    otherFiles.forEach((file) => {
      ...
      document
        .querySelector(".btn-download-file")
        .addEventListener("click", () => {
          downloadFile(file);
        });
