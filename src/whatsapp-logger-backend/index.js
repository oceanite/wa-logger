...
app.get("/api/chats/:remote", async (req, res) => {
  try {
    ...
    if (chatHistory.length === 0) {
      return res
        .status(404)
        .json({ message: `No messages/chat history found for id ${remote}` });
    }
...
app.post("/api/send-file", upload.array("files"), async (req, res) => {
  try {
    ...
    const files = req.files.map((file) => ({
      ...
      path: `${BACKEND_URL}/uploads/${file.filename}`, // Menggunakan filename yang sudah diubah sebelumnya
...
app.get("/api/files/:chatroomID", async (req, res) => {
  try {
    ...
    if (files.length === 0) {
      return res
        .status(404)
        .json({ message: "No files found for this chatroom" });
    }
